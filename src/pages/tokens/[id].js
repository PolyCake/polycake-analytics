...
function TokenPage() {
  ...
  return (
    <AppShell>
      <Head>
        <title>
          {currencyFormatter.format(price || 0)} | {token.symbol} | Polycake
          Analytics
        </title>
      </Head>
      <PageHeader>
        ...
        >
          ...
          <Grid item xs={12} sm="auto" className={classes.links}>
            <Link
              href={`https://exchange.polycake.finance/#/add/${token.id}/ETH`}
              target="_blank"
              variant="body1"
            >
              Add Liquidity
            ...
            <Link
              href={`https://exchange.polycake.finance/#/swap?inputCurrency=${token.id}`}
              target="_blank"
              variant="body1"
            >
              Trade
